...
SELECT info#>'{schema,fields}' FROM dataset.meta WHERE tmp_name=$1;



After it, you can DROP the `tmpcsv_language_codes` FOREIGN TABLE, because you have an exact copy of it into the `dataset.big` table, named `language_codes` in the
