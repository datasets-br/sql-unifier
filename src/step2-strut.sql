...
CREATE TABLE dataset.jtd(
  ...
  name text NOT NULL,  -- JDT label (json-schema URN)
  info JSONb,    -- any metadata as description, etc.
  created date DEFAULT now(),
  UNIQUE(name)
);
INSERT INTO dataset.jtd (name) VALUES ('tabrows'); -- the default datatype!
...
CREATE TABLE dataset.meta (
	...
	jdt text NOT NULL DEFAULT 'tabrows' REFERENCES dataset.jtd(name), -- JSON Schema
...
CREATE TABLE dataset.big (
  ...
  key text,  -- Optional. Dataset primary key (converted to text).
	is_array boolean NOT NULL DEFAULT false, -- cached table into JSONb-array
